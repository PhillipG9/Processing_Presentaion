...
if __name__ == "__main__":
    cre_files = []
    del_files_list = []
    list_of_files = ["test1", "test2", "test3", "test4", "test5", "test6", "test7", "test8", "test9", "test10",
                     "test11"]

    start = time.time()

    for item in list_of_files:
        processes = Process(target=create_files, args=(item, ))
        cre_files.append(processes)
        processes.start()
    ...
    for files in list_of_files:
        del_processes = Process(target=del_files, args=(files,))
        del_files_list.append(del_processes)
        del_processes.start()
